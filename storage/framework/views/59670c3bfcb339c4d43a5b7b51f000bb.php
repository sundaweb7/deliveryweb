...
document.addEventListener('DOMContentLoaded', function(){
    const table = initDataTable('#product-table','<?php echo e(route('admin.products.data')); ?>',[
        { data:0 },{ data:1 },{ data:2 },{ data:3 },{ data:4 },{ data:5 },{ data:6, orderable:false, searchable:false }
    ]);

    $('#btn-add-product').on('click', function(){
        $('#product-form').attr('data-url','<?php echo e(url('admin/products')); ?>').attr('data-method','POST');
        $('#product-form')[0].reset();
        $('#product-image-preview').html('');
        $('#productModal').modal('show');
    });

    $('#product-table').on('click','.btn-delete', function(){if(!confirm('Hapus product?')) return; let id=$(this).data('id'); $.ajax({url:'<?php echo e(url('admin/products')); ?>/'+id, method:'DELETE', headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')}, success:function(){table.ajax.reload()}});
    });

    $('#product-table').on('click','.btn-edit', function(){let id=$(this).data('id'); $.get('<?php echo e(url('admin/products')); ?>/'+id, function(res){$('#product-form').attr('data-url','<?php echo e(url('admin/products')); ?>/'+id).attr('data-method','PUT'); $('#product-mitra_id').val(res.mitra_id); $('#product-category_id').val(res.category_id); $('#product-name').val(res.name); $('#product-price').val(res.price); $('#product-stock').val(res.stock); if (res.image_url) { $('#product-image-preview').html('<img src="'+res.image_url+'" style="height:80px;border-radius:4px">'); } else { $('#product-image-preview').html(''); } $('#productModal').modal('show');});});
});
</script>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\laravel\PROJEK APLIKASI\delivery\resources\views/admin/products/index.blade.php ENDPATH**/ ?>