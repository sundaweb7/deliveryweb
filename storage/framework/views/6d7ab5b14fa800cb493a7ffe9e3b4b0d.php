...
<?php $__env->startSection('title','Mitras'); ?>
<?php $__env->startSection('content'); ?>
            ...
            <div class="modal-content">
                <form class="ajax-form" id="mitra-form" data-refresh-table="#mitra-table">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group"><label>User</label><select name="user_id" id="mitra-user_id" class="form-control"><?php $__currentLoopData = \App\Models\User::where('role','mitra')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($u->id); ?>"><?php echo e($u->name); ?> (<?php echo e($u->email); ?>)</option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function(){
            const table = initDataTable('#mitra-table', '<?php echo e(route('admin.mitras.index')); ?>' + '/data', [
                { data: 0 },
                { data: 1 },
                { data: 2 },
                { data: 3 },
            ...
            $('#btn-add-mitra').on('click', function(){
                $('#mitra-form').attr('data-url','<?php echo e(route('admin.mitras.index')); ?>').attr('data-method','POST');
                $('#mitra-form')[0].reset();
                $('#mitra-id').val('');
            ...
            $('#mitra-table').on('click','.btn-delete', function(){
                ...
                let id = $(this).data('id');
                $.ajax({url: '<?php echo e(url('admin/mitras')); ?>/'+id, method: 'DELETE', headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, success:function(){table.ajax.reload()}});
            });

            $('#mitra-table').on('click','.btn-edit', function(){
                let id = $(this).data('id');
                $.get('<?php echo e(url('admin/mitras')); ?>/'+id, function(res){
                    $('#mitra-form').attr('data-url','<?php echo e(url('admin/mitras')); ?>/'+id).attr('data-method','PUT');
                    $('#mitra-id').val(res.id);
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\laravel\PROJEK APLIKASI\delivery\resources\views/admin/mitras/index.blade.php ENDPATH**/ ?>