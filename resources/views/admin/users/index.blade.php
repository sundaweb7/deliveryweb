...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function(){
            const table = initDataTable('#users-table', '{{ route('admin.users.index') }}' + '/data', [
                { data: 0 },
                { data: 1 },
                { data: 2 },
                { data: 3 },
            ...
            $('#btn-add-user').on('click', function(){
                $('#user-form').attr('data-url','{{ route('admin.users.index') }}').attr('data-method','POST');
                $('#user-form')[0].reset();
                $('#user-id').val('');
            ...
            $('#users-table').on('click','.btn-delete', function(){
                ...
                let id = $(this).data('id');
                $.ajax({url: '{{ url('admin/users') }}/'+id, method: 'DELETE', headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, success:function(){table.ajax.reload()}});
            });

            $('#users-table').on('click','.btn-edit', function(){
                let id = $(this).data('id');
                $.get('{{ url('admin/users') }}/'+id, function(res){
                    $('#user-form').attr('data-url','{{ url('admin/users') }}/'+id).attr('data-method','PUT');
                    $('#user-id').val(res.id);
