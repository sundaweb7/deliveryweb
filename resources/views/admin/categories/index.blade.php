...
<script>
    document.addEventListener('DOMContentLoaded', function(){
        const table = initDataTable('#categories-table','{{ route('admin.categories.data') }}',[
            { data:0 },{ data:1 },{ data:2 },{ data:3 },{ data:4, orderable:false, searchable:false }
        ]);

        $('#btn-add-category').on('click', function(){ $('#category-form').attr('data-url','{{ url('admin/categories') }}').attr('data-method','POST'); $('#category-form')[0].reset(); $('#category-id').val(''); $('#categoryModal').modal('show'); });

        $('#categories-table').on('click','.btn-delete', function(){ if(!confirm('Hapus category?')) return; let id=$(this).data('id'); $.ajax({url:'{{ url('admin/categories') }}/'+id, method:'DELETE', headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')}, success:function(){table.ajax.reload()}});
        });

        $('#categories-table').on('click','.btn-edit', function(){ let id=$(this).data('id'); $.get('{{ url('admin/categories') }}/'+id, function(res){ $('#category-form').attr('data-url','{{ url('admin/categories') }}/'+id).attr('data-method','PUT'); $('#category-id').val(res.id); $('#category-name').val(res.name); $('#category-desc').val(res.description); $('#category-status').val(res.status); $('#categoryModal').modal('show'); }); });
    });
</script>
@endsection