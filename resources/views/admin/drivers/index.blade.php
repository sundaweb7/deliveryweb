...
document.addEventListener('DOMContentLoaded', function(){
    const table = initDataTable('#driver-table','{{ route('admin.drivers.data') }}',[
        { data:0 },{ data:1 },{ data:2 },{ data:3 },{ data:4 },{ data:5 },{ data:6,orderable:false,searchable:false }
    ]);

    $('#btn-add-driver').on('click', function(){
        $('#driver-form').attr('data-url','{{ url('admin/drivers') }}').attr('data-method','POST');
        $('#driver-form')[0].reset();
        $('#driverModal').modal('show');
    });

    $('#driver-table').on('click','.btn-delete', function(){if(!confirm('Hapus driver?')) return; let id=$(this).data('id'); $.ajax({url:'{{ url('admin/drivers') }}/'+id, method:'DELETE', headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')}, success:function(){table.ajax.reload()}});
    });

    $('#driver-table').on('click','.btn-edit', function(){let id=$(this).data('id'); $.get('{{ url('admin/drivers') }}/'+id, function(res){$('#driver-form').attr('data-url','{{ url('admin/drivers') }}/'+id).attr('data-method','PUT'); $('#driver-user_id').val(res.user_id); $('#driver-name').val(res.name); $('#driver-address').val(res.address); $('#driver-wa_contact').val(res.wa_contact); $('#driver-bank_account_number').val(res.bank_account_number); $('#driver-bank_name').val(res.bank_name); $('#driver-bank_account_name').val(res.bank_account_name); $('#driver-vehicle_type').val(res.vehicle_type); $('#driver-vehicle_number').val(res.vehicle_number); $('#driver-active').val(res.is_active?1:0); $('#driverModal').modal('show');});});
});
</script>
@endsection